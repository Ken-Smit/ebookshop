...
<body>
  ...
  if (booklist == null) {
    response.sendRedirect("/ebookshop/index.jsp");
  } else {
  %>
  ...
  <%  // Scriptlet 2: copy the booklist to the selection control
        for (int i = 0; i < booklist.size(); i++) {
          ebookshop.Book aBook = booklist.elementAt(i); 
          out.println("<option value=\"" + aBook.getTitle() + "\">" + aBook.getTitle() + "</option>");
        }
  %>
      </select>
  ...
  <%  // Scriptlet 4: display the books in the shopping cart
    for (int i = 0; i < shoplist.size(); i++) {
      ebookshop.Book aBook = shoplist.elementAt(i);
  %>
      <tr>
